    ...
    public function parseFile($filePath, $year, $mision)
    {
        ...
        foreach ($sheets as $sheet) {
            ...
            if (str_contains(strtolower($sheetTitle), 'ndice') || str_contains(mb_strtolower($sheetTitle, 'UTF-8'), 'ndice')) {
                \Illuminate\Support\Facades\Log::info("=== PROCESANDO HOJA 脥NDICE (ESTRAT脡GICO): $sheetTitle ===");
                ...
                for ($row = 1; $row <= 15; $row++) {
                    ...
                    if (array_search('clave', $tempHeaders) !== false || array_search('codigo', $tempHeaders) !== false || array_search('c贸digo', $tempHeaders) !== false) {
                        $headers = $tempHeaders;
                        $headerRowIndex = $row;
                        break;
                    }
                ...
                if ($headerRowIndex !== -1) {
                    ...
                    foreach ($headers as $col => $header) {
                        ...
                        elseif (str_contains($header, 't铆tulo') || str_contains($header, 'titulo')) $colTitulo = $colTitulo ?: $col;
        ...
        foreach ($sheets as $sheet) {
            ...
            $mergedData = []; // Keyed by col_1 value (e.g. "El C贸digo de 茅tica")
            $notas = [];
            $fuente = null;
            $currentGlobalTitle = '';
            ...
            while ($r <= $highestRow) {
                ...
                // Check if it's a lone title row (like "Sesiones de capacitaci贸n... 2024")
                ...
                for ($row = $dataStartRow; $row <= $endRow; $row++) {
                    $rowKey = $rawGrid[$row][1]; // e.g., "Calakmul" or "El C贸digo de 茅tica"
